...
export const Path = ({ fileInfo, currentDir }) => {
  ...
  if (splitPath.length == 1) {
    // browse
    if (fileInfo.isDir) {
      const target = currentDir == "" ? `/${fileInfo.path}/` : `/${currentDir}/${fileInfo.path}/`;
    ...
    <>
      {splitPath
        .map((val, i) => {
          ...
          const target = currentDir == "" ? `${splitPath.slice(0, i + 1).join("/")}/` : `/${currentDir}/${splitPath.slice(0, i + 1).join("/")}/`;
          return (
            <a
              key={i}
              href={target}
