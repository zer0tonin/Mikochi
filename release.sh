...
#docker buildx build --platform linux/arm/v7 -t zer0tonin/mikochi:latest -t zer0tonin/mikochi:$version --push -f ./Dockerfile-armv7 .
...
echo $release_id
#docker run --env RELEASE=$release_id --env GITHUB_TOKEN=$token --platform linux/arm/v7 -v $(PWD)/tar-upload.sh:/tar-upload.sh -it zer0tonin/mikochi:latest /bin/bash /tar-upload.sh
